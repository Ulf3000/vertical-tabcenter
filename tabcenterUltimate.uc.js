...
tabcenterUltimate();
function tabcenterUltimate() {



	//CustomizableUI.areas.splice(3, 1);
	...
	filterBox.addEventListener("input", filterTabs, false);
	...
	window.messageManager.addMessageListener("tabCenterUltimate@Ulf3000:my-e10s-extension-message", updateThumbnail);
	...
	window.addEventListener("TabAttrModified", onTabAttrModified, false);
	...
	document.addEventListener("DOMContentLoaded", function (event) {
		...
		addImageToTab(gBrowser._tabs[0]); // hack , the first loaded tab in a gBrowser doesnt fire a tabopen event, all other tabs loaded after it do though
	...
	//-----------------------------------------------------------------------------
	//-----------------------------------------------------------------------------
	async function addImageToTab(aTab) {
		await new Promise(resolve => setTimeout(resolve, 300)); //wait a little bit , maybe not needed 
	...
	async function updateThumbnail(message) {   // return the tab  thumbnail
		// console.log(message);
	...
	function openNewTab(e) {
		...
		if (e.button == 2) { //BUTTON 2   add new sibling
			...
			//VerticalTabsReloaded.setTabAttribs(newTab, "indentFactor", indentFactor);
	...
	function onTabSelect(e) {
		...
		// } else {
		// 	console.log(lastBrowser);
		// 	let img = lastActiveTab.getElementsByClassName("ThumbNail_HT")[0];
		// 	let CNVS = document.createElement('canvas');
		// 	CNVS.width = 54;
		// 	CNVS.height = 36
		// 	PageThumbUtils.createSnapshotThumbnail(lastBrowser, CNVS);
		// 	img.src = CNVS.toDataURL();
		// 	console.log(CNVS.toDataURL());
		// 	SessionStore.setCustomTabValue(lastActiveTab, "ThumbNail_HT", img.src);
		// }
	}

	async function onTabOpen(e) {
		// console.log("TAB OPENED !!!!!!!!!!");
		// console.log(e);
		addImageToTab(e.target);
